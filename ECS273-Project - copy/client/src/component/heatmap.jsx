...
function Heatmap() {
  ...
  useEffect(() => {
    if (mode === "time") {
      fetch("http://localhost:8000/heatmap/time")
        .then((res) => res.json())
        .then((data) => {
          setTimeData(data);
          setError(null);
        })
        .catch((e) => setError("time data load failed: " + e.message));
    }
  }, [mode]);
  ...
  const renderZipcodeHeatmap = () => {
    ...
    svg
      ...
      .on("mousemove", (event) => {
        ...
        const offsetX = 10;
        const offsetY = 10;
        tooltip.style("left", (event.clientX - rect.left + offsetX) + "px")
          .style("top", (event.clientY - rect.top + offsetY) + "px")
          .style("opacity", 1);
      })
      .on("mouseleave", (event) => {
  ...
  return (
    <div style={{ position: "relative" }}>
      <h3 style={{color: "blueviolet"}}>Violation Heatmap</h3>
      ...
      <div style={{ marginTop: 20 }}>
        {mode === "time" && <svg id="time-heatmap"></svg>}
