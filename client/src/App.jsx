...
import IntervalFrequencyTrend from "./components/interval_frequency_trend.jsx";
...
function App() {
  ...
  const handleSearchSubmit = () => {
    ...
    const matched = (input.match(/\b\d{5}\b/) || [""])[0];
    setHighlightZip(matched);
  };
    ...
    <div className="flex flex-col h-full w-full">
      <header className="bg-zinc-400 text-white p-2 flex justify-between items-center">
        ...
        <form>
          ...
          <button type = "button" className = "bg-blue-600 text-white px-3 py-1 rounded" id = "submit" onClick={handleSearchSubmit} >Submit</button>
        </form>
      </header>
      ...
      <div className="p-4 mt-4 border-t border-gray-300">
        <h3 className="text-left text-xl mb-2">Interval & Frequency Trend</h3>
        <div className="border-2 border-gray-300 rounded-xl" style={{ height: "400px" }}>
          <IntervalFrequencyTrend />
        </div>
      </div>
    </div>
